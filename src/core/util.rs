...
fn inject_by_yapi(pid: u32, name: &str, dll: &str) {
    unsafe {
        // open remote process
...
#[cfg(target_arch = "x86")]
fn inject_by_wow64ext(pid: u32, name: &str, dll: &str) {
    unsafe {
        // open remote process
        let Ok(h_proc) = OpenProcess(
            PROCESS_CREATE_THREAD
...
fn inject_by_native(pid: u32, name: &str, dll: &str) {
    unsafe {
        // get kernel32 module
...
pub fn inject_to_process(mode: &str, exit: bool, pid: u32, name: String, dll: String, delay: u32) {
    info!("[+] Inject Task...");
    {
